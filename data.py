...
for i, u in enumerate(users):
    auth_users.append((
        u[0],
        f"user{i+1}",
        "hashedpassword",  # Assume all use same hashed password for demo
        "user",
...
cursor.executemany("INSERT INTO auth_users (user_id, username, password_hash, role, last_login, is_active) VALUES (?, ?, ?, ?, ?, ?)", auth_users)
